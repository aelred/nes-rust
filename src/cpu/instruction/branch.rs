...
impl<M: Memory + Tickable> CPU<'_, M> {
    ...
    fn branch_if(&mut self, cond: bool) {
        ...
        if cond {
            let previous = self.state.program_counter;
            self.state.program_counter += offset as u16;
            self.increment_cycle_count();
            if self.state.program_counter.page_crossed(previous) {
                self.increment_cycle_count();
            }
        }
    }
...
mod tests {
    ...
    fn instr_bcc_branches_when_carry_flag_clear() {
        ...
        assert_eq!(cpu.state.program_counter, Address::new(82));
    }
